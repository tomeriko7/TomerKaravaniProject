...
const closeInstructions2 = document.getElementById("closeInstructions"); // כפתור ✖
...
// בוחרים את כל הכפתורים שסוגרים את ההוראות
const closeInstructionsBtns = document.querySelectorAll(
  "#closeInstructions, #closeInstructions2"
);

// מוסיפים לכל אחד event listener
closeInstructionsBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    instructions.style.display = "none";
    localStorage.setItem("instructionsShown", "true"); // כדי לא להציג שוב בפעם הבאה
  });
});

// פותחים את ההוראות בכפתור פתיחה
openInstructions.addEventListener("click", () => {
  instructions.style.display = "block";
});

openInstructions.addEventListener(
  "click",
  () => (instructions.style.display = "block")
);
...
function getRandomFruit() {
  ...
  fruits = fruits.filter(function (val) {
    return val !== randomWord;
  });
...
function getRandomCountry() {
  ...
  wordDisplay.innerHTML = convertToUnderscore(randomWord);
  countries = countries.filter(function (val) {
    return val !== randomWord;
  });
...
function getRandomAnimal() {
  ...
  wordDisplay.innerHTML = convertToUnderscore(randomWord);
  animals = animals.filter(function (val) {
    return val !== randomWord;
  });
